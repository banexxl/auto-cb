...
For ticket-building, only evaluate soccer and basketball candidate selections whose status is `SELECTION_ENABLED`; ignore every other sport or disabled/stale selection. Evaluate every remaining candidate match and both teams/actors before selecting any leg. Use all available markets, current odds, selection status, marketUrl, minStake, maxStake, cutoff time, team news, form, tactical fit, motivation, travel, schedule pressure, weather/surface, market movement, and integrity-risk indicators. Calculate the combined quota by multiplying selected decimal odds. Keep the total quota between 1.5 and 2.2 inclusive. Prefer fewer, stronger legs. Never add a weak selection only to reach the quota range. If no acceptable combination exists, output `NO_BET`.
