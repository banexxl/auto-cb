...
export function SoccerEnabledMatchesButton() {
  const [open, setOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [data, setData] = useState<EnabledSoccerMatchesResponse | null>(null);
  ...
  async function loadMatches() {
    ...
    try {
      const response = await fetch("/api/cloudbet/matches/soccer-enabled", { cache: "no-store" });
      const body = (await response.json()) as EnabledSoccerMatchesResponse & { message?: string };

      if (!response.ok) {
        setErrorMessage(body.message ?? "Unable to load enabled soccer matches.");
        return;
      }

      setData(body);
    } catch {
      setErrorMessage("Unable to load enabled soccer matches.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <>
      <Button onClick={handleOpen} variant="outlined">
        Enabled soccer matches
      </Button>
      <Dialog fullWidth maxWidth="md" onClose={() => setOpen(false)} open={open}>
        <DialogTitle>Soccer matches with SELECTION_ENABLED</DialogTitle>
        <DialogContent>
          <Stack spacing={2} sx={{ py: 1 }}>
            <Stack direction="row" spacing={1} sx={{ justifyContent: "space-between" }}>
              <Typography color="text.secondary" variant="body2">
                {data ? `${data.totalMatches} matches - ${data.totalSelections} enabled selections` : "Fetches current Cloudbet soccer lines."}
              </Typography>
            ...
            {isLoading ? (
              <Stack spacing={1} sx={{ alignItems: "center", py: 4 }}>
                ...
                <Typography color="text.secondary" variant="body2">Loading enabled soccer matches...</Typography>
              </Stack>
            ) : null}
            ...
            {!isLoading && data?.matches.length === 0 ? (
              <Alert severity="info">No soccer matches currently have SELECTION_ENABLED selections.</Alert>
            ) : null}
              ...
              <Box key={match.id} sx={{ border: "1px solid", borderColor: "divider", borderRadius: 3, p: 2 }}>
                <Stack spacing={1}>
                  <Stack direction={{ xs: "column", sm: "row" }} spacing={1} sx={{ justifyContent: "space-between" }}>
                    ...
                    <Button component={Link} href={`/matches/${match.id}?sport=soccer`} size="small">
                      View
                    </Button>
                  </Stack>
                  <Divider />
